...
int main(int argc, char** argv) {
	...
	while (argc > 1) {
		...
		case 'p': { // specify start port number
			int portArg;
			if (sscanf_s(argv[2], "%d", &portArg) != 1) {
				usage();
			}
		...
		case 'D': { // specify maximum number of seconds to wait for packets:
			if (sscanf_s(argv[2], "%u", &interPacketGapMaxTime) != 1) {
				usage();
			}
			++argv; --argc;
			break;
		...
		case 'S': { // specify an offset to use with "SimpleRTPSource"s
			if (sscanf_s(argv[2], "%d", &simpleRTPoffsetArg) != 1) {
				usage();
			}
		...
		case 'P': { // specify an interval (in seconds) between writing successive output files
			...
			if (sscanf_s(argv[2], "%d", &fileOutputIntervalInt) != 1 || fileOutputIntervalInt <= 0) {
				usage();
			}
		...
		case 'T': {
			...
			if (controlConnectionUsesTCP) {
				if (argc > 3 && argv[2][0] != '-') {
					...
					if (sscanf_s(argv[2], "%hu", &tunnelOverHTTPPortNum) == 1
		...
		case 'u': { // specify a username and password
			...
			if (allowProxyServers && argc > 3 && argv[2][0] != '-') {
				...
				if (argc > 3 && argv[2][0] != '-') {
					...
					if (sscanf_s(argv[2], "%hu", &proxyServerPortNum) != 1) {
						usage();
					}
					++argv; --argc;
				}
		...
		case 'A': { // specify a desired audio RTP payload format
			unsigned formatArg;
			if (sscanf_s(argv[2], "%u", &formatArg) != 1
				|| formatArg >= 96) {
					usage();
			}
		...
		case 'w': { // specify a width (pixels) for an output QuickTime or AVI movie
			if (sscanf_s(argv[2], "%hu", &movieWidth) != 1) {
				usage();
			}
		...
		case 'h': { // specify a height (pixels) for an output QuickTime or AVI movie
			if (sscanf_s(argv[2], "%hu", &movieHeight) != 1) {
				usage();
			}
		...
		case 'f': { // specify a frame rate (per second) for an output QT or AVI movie
			if (sscanf_s(argv[2], "%u", &movieFPS) != 1) {
				usage();
			}
		...
		case 'b': { // specify the size of buffers for "FileSink"s
			if (sscanf_s(argv[2], "%u", &fileSinkBufferSize) != 1) {
				usage();
			}
			++argv; --argc;
			break;
		...
		case 'B': { // specify the size of input socket buffers
			if (sscanf_s(argv[2], "%u", &socketInputBufferSize) != 1) {
				usage();
			}
			++argv; --argc;
			break;
		...
		case 'Q': { // output QOS measurements
			...
			if (argc > 3 && argv[2][0] != '-') {
				...
				if (sscanf_s(argv[2], "%u", &qosMeasurementIntervalMS) != 1) {
					usage();
				}
		...
		case 's': { // specify initial seek time (trick play)
			double arg;
			if (sscanf_s(argv[2], "%lg", &arg) != 1 || arg < 0) {
				usage();
			}
			initialSeekTime = arg;
			++argv; --argc;
		...
		case 'z': { // scale (trick play)
			float arg;
			if (sscanf_s(argv[2], "%g", &arg) != 1 || arg == 0.0f) {
				usage();
			}
			scale = arg;
			++argv; --argc;
		...
		case 'R': {
			...
			if (argc > 2 && argv[2][0] != '-') {
				...
				if (sscanf_s(argv[2], "%hu", &handlerServerForREGISTERCommandPortNum) == 1 && handlerServerForREGISTERCommandPortNum > 0) {
					++argv; --argc;
					break;
				}
			}
	...
	*env << "Warning: Because we're not receiving stream data, the -n flag has no effect\n";
	...
	streamURL = argv[1];
	*/
	streamURL = "rtsp://192.168.1.1/h264?w=320&h=240&fps=30&br=400000";
...
void createOutputFiles(char const* periodicFilenameSuffix) {
	...
	if (outputQuickTimeFile || outputAVIFile) {
		if (periodicFilenameSuffix[0] == '\0') {
			...
			sprintf_s(outFileName, sizeof outFileName, "%s%s.%s", prefix, periodicFilenameSuffix,
		...
		while ((subsession = iter.next()) != NULL) {
			...
			if (singleMedium == NULL || periodicFilenameSuffix[0] != '\0') {
				...
				sprintf_s(outFileName, sizeof outFileName, "%s%s-%s-%d%s",
			...
			if (fileSink != NULL) subsession->sink = fileSink;
...
void createPeriodicOutputFiles() {
	...
	sprintf_s(periodicFileNameSuffix, sizeof periodicFileNameSuffix, "-%05d-%05d",
...
void checkForPacketArrival(void* /*clientData*/) {
	...
	if (notifyTheUser) {
		...
		sprintf_s(timestampStr, "%ld%03ld", timeNow.tv_sec, (long)(timeNow.tv_usec/1000));
